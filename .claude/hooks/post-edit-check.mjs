#!/usr/bin/env node
// PostToolUse(Write|Edit) — 편집 직후 불변식 위반을 즉시 되돌려준다. INV-1, INV-2, INV-4, INV-5.
// 권위 있는 검사는 'npm run lint:arch'. 이 훅은 빠른 피드백용이다.
import { readFileSync, existsSync } from 'node:fs';
import { readInput, relPath, block, ok, setState } from './_lib.mjs';
...
lines.forEach((l, i) => {
  ...
  if (/(:|<|\bas\s+)\s*any\b/.test(l) && !/\/\/.*any/.test(l)) escapes.push(`${i + 1}: any`);
});
if (escapes.length) {
  problems.push(
...
if (m) {
  ...
  if (li !== -1) {
    ...
    for (const spec of imports) {
      ...
      const otherDomain = abs.match(/(?:^|\/)domains\/([^/]+)/);

      if (otherDomain && otherDomain[1] !== domain) {
        problems.push(
          `[INV-2] ${file} 이 다른 도메인 '${otherDomain[1]}' 을 직접 import 한다: ${spec}\n` +
            `  고치는 법: 공유가 필요하면 shared/ 로 승격하라. 도메인 간 직접 참조는 금지다.`,
        );
      } else if (up) {
        const ti2 = LAYERS.indexOf(up[1]);
        if (ti2 > li) {
          problems.push(
